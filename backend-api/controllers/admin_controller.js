...
exports.getPendingCategories = async (req, res) => {
    try {
        ...
        console.error('Error fetching pending categories:', err.message);
        res.status(500).json({ error: 'Failed to retrieve pending categories.', details: err.message });
    }
};
...
exports.approveCategory = async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            ...
            WHERE category_id = $1
            RETURNING category_id, category_name, is_approved
        `, [categoryId]);
        ...
        res.status(500).json({ error: 'Failed to approve category.', details: err.message });
    }
};
